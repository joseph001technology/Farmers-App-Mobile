...
class _ConsumerDashboardScreenState extends State<ConsumerDashboardScreen>
  ...
  // ── FIXED: Future<void> so RefreshIndicator.onRefresh is satisfied ──
  ...
  List<MapEntry<String, double>> get _topFarmers {
    ...
    for (final o in _orders) {
      // ── FIXED: farmerName now exists on Order model ──────────────
      final name = o.farmerName ?? 'Unknown Farmer';
        ...
        children: [
          ...
          GridView.count(
            ...
            children: [
              ...
              _statTile(
                  '✅', 'Delivered', '$_deliveredCount', Colors.green),
              _statTile(
  ...
  // ════════════════════════════════════════════════════════════════
  Widget _farmersTab() {
    if (_topFarmers.isEmpty) {
      return _emptyState('🌿', 'No farmers yet',
          'Farmers you buy from will appear here');
    }

    final maxSpend = _topFarmers.first.value;
    final farmerOrders = <String, int>{};
    for (final o in _orders) {
      final name = o.farmerName ?? 'Unknown Farmer'; // ← fixed ✓
      farmerOrders[name] = (farmerOrders[name] ?? 0) + 1;
    }
      ...
      child: ListView(
        ...
        children: [
          _summaryBanner(
            emoji: '🌾',
            title:
                '${_topFarmers.length} farmer${_topFarmers.length == 1 ? '' : 's'} supported',
            subtitle: 'You\'re helping local agriculture grow',
            color: Colors.green,
          ),
          const SizedBox(height: 20),
          Text('Farmers you buy from most',
              style: GoogleFonts.poppins(
                  fontSize: 16, fontWeight: FontWeight.bold)),
          const SizedBox(height: 12),
          ..._topFarmers.asMap().entries.map((entry) {
            final rank = entry.key + 1;
            final farmer = entry.value.key;
            final spend = entry.value.value;
            final orders = farmerOrders[farmer] ?? 0;
            ...
            return Container(
              ...
              child: Column(
                ...
                Row(children: [
                  ...
                  Text(
                      '${(spend / (maxSpend > 0 ? maxSpend : 1) * 100).toStringAsFixed(0)}%',
                      style: GoogleFonts.poppins(
          ...
          if (_topFarmers.isNotEmpty)
            _infoCard(
              ...
              value: _topFarmers.first.key,
              color: Colors.pink,
            ),
          const SizedBox(height: 12),
          _infoCard(
            icon: Icons.people_outline_rounded,
            label: 'Unique farmers supported',
            value: '${_topFarmers.length}',
            color: Colors.teal,
          ),
          const SizedBox(height: 30),
        ],
  ...
  // ════════════════════════════════════════════════════════════════
  // REUSABLE WIDGETS  (all original — zero functionality removed)
  // ════════════════════════════════════════════════════════════════

  Widget _statTile(
          String emoji, String label, String value, MaterialColor color) =>
      Container(
  ...
  Widget _spendBarChart() {
    ...
    final maxAmt = monthly.fold<double>(
        0,
        (m, e) => (e['amount'] as double) > m
            ? (e['amount'] as double)
            : m);
    ...
    return Container(
      ...
      child: Column(
        children: statuses.entries.map((e) {
          ...
          final ratio =
              _orders.isEmpty ? 0.0 : count / _orders.length;
          return Padding(
