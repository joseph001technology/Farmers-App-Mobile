...
class _OrderDetailScreenState extends State<OrderDetailScreen>
  ...
  // UPDATED PDF GENERATION LOGIC WITH BRANDING
        ...
        build: (pw.Context context) {
          return pw.Padding(
            ...
            child: pw.Column(
              ...
              children: [
                // --- AGRIFLOW BRANDING SECTION ---
                pw.Row(
                  crossAxisAlignment: pw.CrossAxisAlignment.center,
                  children: [
                    // LOGO: A stylized green leaf
                    pw.Container(
                      width: 40,
                      height: 40,
                ...
                pw.Row(
                  ...
                  children: [
                    pw.Column(
                      crossAxisAlignment: pw.CrossAxisAlignment.start,
                      children: [
                        pw.Text("RECEIPT FOR:", style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 10)),
                        pw.Text("Order #${order!.id}", style: pw.TextStyle(fontSize: 14, fontWeight: pw.FontWeight.bold)),
                      ],
                    ),
                    pw.Container(
        ...
        children: [
          ...
          if (order!.orderItems.isEmpty)
            Container(
              ...
              child: Center(
                child: Text("No item details available", style: GoogleFonts.poppins(color: Colors.grey[500])),
              ),
            )
          else
                  ...
                  children: [
                    ClipRRect(
                      ...
                      child: item.productImage != null && item.productImage!.isNotEmpty
                          ? Image.network(item.productImage!, width: 56, height: 56, fit: BoxFit.cover, errorBuilder: (c, e, s) => _imgPlaceholder())
                          : _imgPlaceholder(),
                    ),
                    ...
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(item.productName, style: GoogleFonts.poppins(fontWeight: FontWeight.w600, fontSize: 14)),
                          Text("Qty: ${item.quantity}  •  KSh ${item.price.toStringAsFixed(0)} each", style: GoogleFonts.poppins(fontSize: 12, color: Colors.grey[500])),
                        ],
                      ),
                    ),
                    Text("KSh ${(item.price * item.quantity).toStringAsFixed(0)}", style: GoogleFonts.poppins(fontWeight: FontWeight.bold, fontSize: 14, color: Colors.green[700])),
                  ],
                ),
              ),
            ),
          ...
          Container(
            ...
            child: Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Text("Order Total", style: GoogleFonts.poppins(color: Colors.white70, fontSize: 14)),
                Text("KSh ${order!.totalPrice.toStringAsFixed(0)}", style: GoogleFonts.poppins(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 22)),
              ],
            ),
          ),
          const SizedBox(height: 20),
          if (order!.isDelivered) ...[
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                label: Text("Rate & Review Products", style: GoogleFonts.poppins(fontWeight: FontWeight.w600, fontSize: 15)),
                style: ElevatedButton.styleFrom(backgroundColor: Colors.amber[600], foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14))),
              ),
            ),
            const SizedBox(height: 10),
          ],
  ...
  Widget _buildReceiptTab() {
    final canShow = order!.status == 'paid' || order!.status == 'delivered' || order!.status == 'pending_delivery';
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text('Receipt not available yet', style: GoogleFonts.poppins(fontSize: 16, fontWeight: FontWeight.w600, color: Colors.grey[500])),
            const SizedBox(height: 8),
            Text('Receipt will appear here once\nyour payment is confirmed.', textAlign: TextAlign.center, style: GoogleFonts.poppins(fontSize: 13, color: Colors.grey[400])),
          ],
        ),
      );
    }
      ...
      child: Column(
        children: [
          Container(
            ...
            child: Column(
              children: [
                Text("ORDER RECEIPT", style: GoogleFonts.poppins(fontSize: 14, fontWeight: FontWeight.w800, color: Colors.green, letterSpacing: 1.2)),
                const SizedBox(height: 20),
                Text('Order #${order!.id}', style: GoogleFonts.poppins(fontWeight: FontWeight.bold, fontSize: 22)),
                const SizedBox(height: 4),
                Text(_formatDate(order!.createdAt), style: GoogleFonts.poppins(fontSize: 12, color: Colors.grey[500])),
                const SizedBox(height: 20),
                ...
                const SizedBox(height: 8),
                _quickRow('Payment Via', order!.paymentMethod == 'pod' ? 'Pay on Delivery' : 'M-Pesa'),
                const SizedBox(height: 8),
                ...
                ...order!.orderItems.map((item) => Padding(
                  ...
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      Text("${item.productName} x${item.quantity}", style: GoogleFonts.poppins(fontSize: 13, color: Colors.grey[700])),
                      Text("KSh ${(item.price * item.quantity).toStringAsFixed(0)}", style: GoogleFonts.poppins(fontSize: 13, fontWeight: FontWeight.w600)),
                    ],
                  ),
                )),
                ...
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Text('TOTAL PAID', style: GoogleFonts.poppins(fontWeight: FontWeight.bold, fontSize: 16)),
                    Text('KSh ${order!.totalPrice.toStringAsFixed(0)}', style: GoogleFonts.poppins(fontWeight: FontWeight.w900, fontSize: 24, color: Colors.green[700])),
                  ],
                ),
                const SizedBox(height: 10),
                Text("Support local agriculture with every purchase.", style: GoogleFonts.poppins(fontSize: 11, fontStyle: FontStyle.italic, color: Colors.grey[500])),
              ],
            ),
          ),
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              label: Text('Download / Print PDF', style: GoogleFonts.poppins(fontWeight: FontWeight.w700, fontSize: 16)),
              style: ElevatedButton.styleFrom(
                backgroundColor: Colors.green[700], 
                foregroundColor: Colors.white, 
                elevation: 4,
