...
class _FarmerDashboardScreenState extends State<FarmerDashboardScreen>
  ...
  void initState() {
    ...
    _tabs.addListener(() => setState(() {})); // rebuild FAB on tab change
    _loadAll();
  }
  ...
  /// Fetch orders and enrich each with its items list by calling
  /// GET /orders/{id}/ for each order that has no items embedded.
  Future<void> _loadOrders() async {
    setState(() => loadingOrders = true);
    try {
      final res = await http.get(
          Uri.parse('$_base/orders/'), headers: _headers);
      if (res.statusCode == 200) {
        final List raw = jsonDecode(res.body) as List;
        final orders = raw.cast<Map<String, dynamic>>();

        // Enrich orders that have no items by fetching detail
  ...
  // ── Total revenue computed from orders if stats API unavailable ──
  ...
  int get _deliveredCount =>
      ...
      myOrders.where((o) => o['status'] == 'delivered').length;
      ...
      child: ListView(
        ...
        children: [
            ...
            child: Column(
              ...
              children: [
                ...
                if (statsError.isNotEmpty)
                  Text(
                    "Calculated from paid/delivered orders",
                    style: GoogleFonts.poppins(
                ...
                Row(
                  ...
                  children: [
                    _heroStat("Total Orders", "$_totalOrderCount"),
                    _vDiv(),
                    _heroStat("Pending", "$_pendingCount",
                        color: Colors.orange[200]!),
                    _vDiv(),
                    _heroStat("Delivered", "$_deliveredCount",
          ...
          GridView.count(
            ...
            children: [
              _statTile("🛒", "Products Listed",
                  "${myProducts.length}", Colors.blue),
              _statTile("📦", "Total Orders",
                  "$_totalOrderCount", Colors.green),
              _statTile("⏳", "Pending", "$_pendingCount",
                  Colors.orange),
              _statTile("✅", "Delivered", "$_deliveredCount",
                  Colors.teal),
            ],
          ),
          ...
          if (myProducts.isNotEmpty) ...[
            ...
            ...myProducts.take(5).map((p) => _miniProductRow(p)),
          ],
  ...
  Widget _orderCard(Map<String, dynamic> o) {
    ...
    // Customer info — may come from different field names
    final consumerName = o['consumer_name'] ??
        o['consumer_username'] ??
        o['buyer_name'] ??
        o['user_name'];

    final consumerPhone = o['consumer_phone'] ??
        o['buyer_phone'] ??
        o['phone'];

    final deliveryAddr = o['delivery_address'] ??
        o['address'] ??
        o['location'];
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    CircleAvatar(
                      ...
                      child: Text(
                        consumerName != null &&
                                consumerName.toString().isNotEmpty
                            ? consumerName.toString()[0].toUpperCase()
                            : "?",
                        style: GoogleFonts.poppins(
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(
                            consumerName?.toString() ?? "Unknown customer",
                            style: GoogleFonts.poppins(
                                fontWeight: FontWeight.w600,
                                fontSize: 13),
                          ),
                          if (consumerPhone != null)
                            Text(consumerPhone.toString(),
                                style: GoogleFonts.poppins(
                                    fontSize: 12,
                                    color: Colors.grey[500])),
                        ],
                ...
                if (deliveryAddr != null &&
                    deliveryAddr.toString().isNotEmpty) ...[
                  const SizedBox(height: 6),
                  Row(
                    children: [
          ...
          if (rawItems.isNotEmpty) ...[
            ...
            ...rawItems.map((item) {
              ...
              final name = itemMap['product_name'] ??
                  itemMap['name'] ??
                  'Product #${itemMap['product'] ?? itemMap['product_id']}';
              final qty = itemMap['quantity'] ?? 1;
              final price = double.tryParse(
                      itemMap['unit_price']?.toString() ??
                          itemMap['price']?.toString() ??
                          '0') ??
                  0.0;
              final imgUrl = itemMap['product_image'] ??
                  itemMap['image'] ??
                  itemMap['image_url'];
  ...
  Widget _fullProductRow(Map<String, dynamic> p) {
    final stock = p['stock'];
    final stockVal =
        stock != null ? int.tryParse(stock.toString()) ?? 0 : 0;
    final name = p['name']?.toString() ?? 'Product';
    final price =
        double.tryParse(p['price']?.toString() ?? '0') ?? 0.0;
    final id = p['id'];
        ...
        children: [
          ClipRRect(
            ...
            child: (p['image'] != null &&
                ...
                ? Image.network(p['image'].toString(),
                    width: 52,
                    height: 52,
                    fit: BoxFit.cover,
                    errorBuilder: (c, e, s) => _pImgPlaceholder())
                : _pImgPlaceholder(),
          ),
          const SizedBox(width: 12),
          Expanded(
          ...
          if (id != null)
            IconButton(
              ...
              onPressed: () =>
                  _showStockDialog(id, name, stockVal),
            ),
        ],
      ),
    );
  ...
  Widget _pImgPlaceholder() {
    return Container(
      width: 52,
      height: 52,
      color: Colors.green[50],
      child: const Center(
          child: Icon(Icons.grass, color: Colors.green, size: 24)),
    );
  }
            ...
            ElevatedButton(
              onPressed: saving
                  ...
                  : () async {
                      ...
                      setS(() => saving = true);
                      final ok = await DashboardService.updateStock(
                          id is int
                              ? id
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text(ok
                              ? "Stock updated to $newVal ✅"
                              : "Updated locally (server returned error)"),
                          backgroundColor: ok
                              ? Colors.green[700]
                              : Colors.orange[700],
                      ...
                      setState(() {
                        final idx = myProducts
                            .indexWhere((p) => p['id'] == id);
                        if (idx >= 0) {
                          myProducts[idx] =
                              Map.from(myProducts[idx])
                                ..['stock'] = newVal;
                        }
                      });
                    },
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 10),
                _field(priceCtrl, "Price (KSh)",
                    Icons.payments_outlined,
                    type: TextInputType.number),
                const SizedBox(height: 10),
                _field(stockCtrl, "Stock Quantity",
                    Icons.inventory_outlined,
                    type: TextInputType.number),
                const SizedBox(height: 10),
                _field(unitCtrl, "Unit (e.g. kg, bunch)",
